...
/* ─── Per-variant price tables (SoundCloud) ───
   Mirrors the Spotify lookup pattern. When the dropdown matches one of these
   variant names AND the active qty pill matches a listed tier, the lookup
   wins over the legacy base × modifier calc. Tiers not listed here fall
   back to that legacy path. Tier labels MUST match qty-pill .textContent. */
const SOUNDCLOUD_VARIANT_PRICES = {
  // ── Plays (pills: 1K / 5K / 10K / 50K / 100K / 500K / 1M) ──
  'USA Plays':          { '1K': 6,  '5K': 12,  '10K': 20,  '50K': 115, '100K': 199, '500K': 810,  '1M': 1399 },

  // ── Followers (pills: 100 / 500 / 1K / 5K / 10K / 50K / 100K) ──
  'Real Followers':     { '100': 10, '500': 20, '1K': 35, '5K': 99, '10K': 189 },

  // ── Likes (pills: 50 / 100 / 500 / 1K / 5K / 10K / 50K / 100K) ──
  'Real Likes':         { '100': 9, '500': 35, '1K': 65, '5K': 310, '10K': 599 },

  // ── Reposts (pills: 50 / 100 / 500 / 1K / 5K / 10K / 50K / 100K) ──
  'Real Reposts':       { '100': 10, '500': 20, '1K': 30 }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updatePrice(container) {
    ...
    // 1) Try per-variant lookup tables first (Spotify + SoundCloud)
    if (select && activeQty && select.selectedIndex >= 0) {
      ...
      const tables = [];
      if (typeof SPOTIFY_VARIANT_PRICES !== 'undefined')    tables.push(SPOTIFY_VARIANT_PRICES);
      if (typeof SOUNDCLOUD_VARIANT_PRICES !== 'undefined') tables.push(SOUNDCLOUD_VARIANT_PRICES);
      for (const tableSet of tables) {
        const tbl = tableSet[variantName];
        if (tbl && tbl[tierLabel] !== undefined) {
          ...
          // that don't appear in the lookup tables.
